...
import type { WeekdayPerf } from '../../types'
import { fCurrency } from '../../utils/formatters'

export default function WeekdayChart({ data }: { data: WeekdayPerf[] }) {
    ...
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 4 }}>
        ...
        <Bar dataKey="avgPnl" radius={[6, 6, 0, 0]}>
          {data.map((d, i) => <Cell key={i} fill={d.avgPnl >= 0 ? 'var(--profit)' : 'var(--loss)'} fillOpacity={0.7} />)}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  )
